...
\begin{algorithm}
	...
	\Begin{
		...
		Compute $a_0,b_0\in R[x]$ such that $$a_0=s_0e+f_0\sum\limits_{i=1}^m p_i,\qquad v(zb_0-a_0)\leq \eps\gm,\qquad  \deg b_0\leq \deg f- \deg\prod\limits_{i=1}^m f_i$$\;
		\For{$0\leq i\leq m$}{
		$f_i^*=f_i+b_i$
		}
		\For{$1\leq i\leq m$}{
...
In the following section we will prove that the algorithm described in the above section gives us the $\mcF^*$ and $T$ which if you put in the Hensel Lemma  for Linear Case and Quadratic Case the conditions are hold.

\begin{proof-of-theorem}{th:hslem}
	Given that $v(e)\leq \eps$. For $i\in [m]$ we have \[
		 s_ie=p_if_i+a_i
	\]where $f_i$ is monic and $\deg a_i\leq \deg f_i$. By \thmref{th:hlm} $v(s_ie)\geq v(a_i)$, and $v(s_i)\geq v(p_i)$. And \[
	...
	\end{itemize}
	\nt{	This proves:\begin{itemize}
			\item  	$H_3(m,f,\mcF^*,\mcS^*,z,\alpha,\delta^*,\eps^*)$ for both Linear and Quadratic case 
			\item $H_4(m,f,\mcF,\mcF^*,\mcS,\mcS^*,\alpha,\delta,\eps)$ for Linear Case.
		\end{itemize}}
	\vspace{3mm}
	
	Now $g_i=\prod\limits_{j\neq i}f_i$ for $0\leq i\leq m$.  Then  we have \begin{align*}
		a_0g_0=s_0g_0+f_0g_0\sum_{i=1}^mp_i & = s_0g_0e+\prod_{i=0}^mf_i \sum_{i=1}^mp_i\\
		& = s_0g_0e+\sum_{i=1}^m g_if_ip_i\\
		& = s_0g_0e+\sum_{i=1}^m g_i(s_e-a_i)\\
		& = e\sum_{i=0}^, s_ig_i-\sum_{i=1}^ma_ig_i\\
		& =e\underbrace{\lt[ \sum_{i=0}^ms_ig_i-z \rt]}_{u_1}+\underbrace{\lt[ ze-\sum_{i=1}^ma_ig_i \rt]}_{u_2}
	\end{align*}
Given $v\lt(  \sum\limits_{i=0}^ms_ig_i-z\rt)\leq \delta$ by $H_2(m,f,\mcF,\mcS,z,\delta)$. So we have $$v\lt(  e\sum\limits_{i=0}^ms_ig_i-z\rt)=v(e)v\lt(  \sum\limits_{i=0}^ms_ig_i-z\rt)\leq \eps\delta$$
