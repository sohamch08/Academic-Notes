...
At this point suppose the graph $G''$ is now $D$ regular. Then we take an expander graph $H$ which is $\lt(D^4,D,\frac34\rt)$. Now we will recursively define a graph family$$G_0=G'',\qquad G_{k}=(G_{k-1}\textcircled{z}H)^8$$
}
\end{document}
