...
\item Consider the set $\spec(V)_T=\{U\subseteq V\mid \text{$U$ subspace of $V$ containing $T$}\}$ for any vector space $V$ over $\bbF$. Now we have to show there is a bijection between $\spec(V)_T$ and $\spec\lt(\quotient{V}{T}\rt)$. Consider the function $f:\spec(V)_T\to \spec\lt(\quotient{V}T\rt)$ where $U\mapsto \quotient{U}{T}$ for any subspace $U\in \spec{V}_T$. Now we will show $f$ is a bijection. \begin{itemize}
	\item \textbf{Injectivity:} Now let $U,W\in \spec(W)_T$ such that $f(U)=f(W)$. Hence we have $\quotient{U}{T}=\quotient{W}{T}$. Let $u\in U$. Then $u+T\in \quotient{U}{T}$. Therefore $u+T\in \quotient{W}{T}$. So $u+T=w+T$ for some $w\in W$. Now we have $u\in w+T\subseteq w+W=W$. So $u\in W$. Therefore $U\subseteq W$. Similarly for any $w\in W$ we have $$w+T\in \quotient{W}{T}\implies w+T\in \quotient{U}{T}\implies w+T=u+T\text{ for some $u\in U$}\implies w\in u+T\subseteq u+U=U$$Hence $W\subseteq U$. Therefore we have $U=W$. Hence $f$ is injective.
	\item 
\end{itemize}
\end{enumerate}
}
