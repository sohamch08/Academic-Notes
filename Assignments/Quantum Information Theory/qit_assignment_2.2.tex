...
\solve{
	\begin{itemize}
		...
		\item Let $T=\sum\limits_{i,j}\gm_{i,j}A_i\tensor B_j$ where $\gm_{i,j}\in\bbC$ and $A_i\in \sL(\mcH_1)$, $B_j\in\sL(\mcH_2)$. Then \begin{align*}
...
\end{problem}
\solve{
Let $\dim(\mcH_A)=d$, $\dim(\mcH_{R_1})=d_1$ and $\dim(\mcH_{R_2})=d_2$. Then suppose $\{\ket{a_i}\mid i\in [d]\}$is an ONB and eigenbasis of $\mcH_{A}$ with corresponding eigenvalues are $\{\alpha_i\mid i\in[d]\}$. Also suppose $\{\ket{e_i}\mid i\in [d_1] \}$ is an ONB of $\mcH_{R_1}$ and $\{ \ket{f_i}\mid i\in[d_2] \}$ is an ONB of $\mcH_{R_2}$. We can express $$\rho_A=\sum_{i=1}^d\alpha_i\ket{a_i}\bra{a_i}$$

For now we assume $K=L=d$.  Now take $\min\{d,d_1\}=K$ and $\min\{d,d_2\}=L$. Therefore $K\leq L$ since $d_1\leq d_2$. Then by Schmidt Decomposition there exists $\lm_j\in \bbC$ $\forall\ j\in[k]$ and $\gm_j\in\bbC$, $\forall\ j\in [L]$ such that $$\ket{\psi}_{AR_1}=\sum_{i=1}^K\lm_i\ket{a_i}_A\tensor \ket{e_i}_{R_1}\qquad\text{and}\qquad  \ket{\psi}_{AR_2}=\sum_{i=1}^L\gm_i\ket{a_i}_A\tensor \ket{f_i}_{R_2}$$Then we have 
Since \begin{align*}
	tr_{R_1}(\ket{\psi}\bra{\psi}_{AR_1}) & = tr_{R_1}\lt[\lt( \sum_{i=1}^K\sqrt{p_X(x)}\ket{e_x}_{R_1}\tensor \ket{x}_A \rt)\lt(\sum_{j=1}^K\sqrt{p_X(x')}\bra{e_{x'}}_{R_1}\tensor \bra{x'}_A \rt)\rt]\\
	& = tr_{R_1}\lt[\sum_{i=1}^K\sum_{j=1}^K\sqrt{p_X(x)p_X(x')}\ket{e_x}\bra{e_{x'}}_{R_1}\tensor \ket{x}\bra{x'}_A  \rt]\\
	& =\sum_{i=1}^K\sum_{j=1}^K\sqrt{p_X(x)p_X(x')}\ket{x}\bra{x'}_A tr(\ket{e_x}\bra{e_{x'}}_{R_1}) \\
	& = \sum_{i=1}^K\sqrt{p_X(x)p_X(x)}\ket{x}\bra{x}_A  =\sum\limits_{i=1}p_X(x)\ket{x}\bra{x}_A=\rho_A
\end{align*}
Similarly\begin{align*}
	tr_{R_2}(\ket{\psi}\bra{\psi}_{AR_2}) & = tr_{R_2}\lt[\lt( \sum_{i=1}^K\sqrt{p_X(x)}\ket{f_x}_{R_2}\tensor \ket{x}_A \rt)\lt( \sum_{j=1}^K\sqrt{p_X(x')}\bra{f_{x'}}_{R_2}\tensor \bra{x'}_A \rt)\rt]\\
	& = tr_{R_2}\lt[\sum_{i=1}^K\sum_{j=1}^K\sqrt{p_X(x)p_X(x')}\ket{f_x}\bra{f_{x'}}_{R_2}\tensor \ket{x}\bra{x'}_A  \rt]\\
	& = \sum_{i=1}^K\sum_{j=1}^K\sqrt{p_X(x)p_X(x')}\ket{x}\bra{x'}_A  tr(\ket{f_x}\bra{f_{x'}}_{R_2}) \\
	& = \sum_{i=1}^K\sqrt{p_X(x)p_X(x)}\ket{x}\bra{x}_A  =\sum\limits_{x\in \mcX}p_X(x)\ket{x}\bra{x}_A=\rho_A
\end{align*}Now take the isometry $V$ to be $$V=\sum_{i=1}^d\ket{e_i}_{R_1}\bra{f_i}_{R_2}$$
}
...
\solve{
\begin{enumerate}
	\item Let $\{\ket{c_l}\mid 1\leq l\leq L\}$ be an ONB of $\mcH_A$. Since $g(Z)\in \sL(\mcH_A)$ we the $(i,j)$th element of $g(Z)$ with respect to the basis $\{\ket{c_l}\mid 1\leq l\leq L\}$ is the value of $\bra{c_i}g(Z)\ket{c_j}$. Now Let $T=\sum\limits_{i,j}\gm_{i,j}A_i\tensor B_j$ then $\forall\ m,m\in [d_A]$ \begin{align*}
