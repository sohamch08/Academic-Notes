...
\begin{enumerate}[label=(\arabic*)]
	\item $\sE:\bbC\to \sD(\mcH)$ where $1\mapsto \rho$. This is indeed a CPTP map. Let $\rho = \sum\limits_{t=1}^d\lm_t\ket{v_t}\bra{v_t}$. the Choi-Kraus Operator is $$E_t:\bbC\to \sL(\mcH), \ 1\mapsto \sum_{t=1}^d\sqrt{\lm_t}\ket{v_t}$$
		Then \begin{align*}
			\sum_{t=1}^dE_tE_t^{\dagger}&= \sum_{t=1}^dE_tE_t^{\dagger}=\sum_{t=1}^dE_t^{\dagger}1E_t\\
			& = \sum_{t=1}^d \sqrt{\lm_t}\ket{v_t}\sum_{t'=1}^d \sqrt{\lm_{t'}}\bra{v_{t'}}\\
			& = \sum_{t=1}^d \lm_t\ket{v_t}\bra{v_t}=\rho=\sE(1)
		\end{align*}And 
\begin{align*}
	\sum_{t=1}^dE_t^{\dagger}E_t&= \sum_{t=1}^dE_t^{\dagger}E_t=\sum_{t=1}^dE_t1E_t\\
	& = \sum_{t=1}^d \sqrt{\lm_t}\bra{v_t}\sum_{t'=1}^d \sqrt{\lm_{t'}}\ket{v_{t'}}\\
	& = \sum_{t=1}^d \lm_t\bra{v_t}\ket{v_t}=1=I_{\bbC}
\end{align*}
Now take $|mcH_A=\bbC$ and $\mcH_R=\bbC^m$ where $m\leq \dim(\mcH)$. Let $\{\ket{g_k}\mid 1\leq k\leq m\}$ is an ONB of $\mcH_R$. Hence  take the isometry $V:\mcH_A\to \mcH_A\tensor \mcH_R$,  where for any $\ket{v}\in \mcH_A$, $\ket{v}\mapsto \sum_{k=1}^m E_t\ket{v}\tensor ket{g_k} $
