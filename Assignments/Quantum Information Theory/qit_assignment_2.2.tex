...
\end{problem}
\solve{
	Let $\{\ket{e_k}\mid 1\leq k\leq K\}$ be an ONB of $\mcH_1$ and $\{\ket{f_l}\mid 1\leq l\leq L\}$ be an ONB of $\mcH_2$. Now given that $A\geq 0, B\geq 0$. By \pref{2} we have that $\{\ket{e_k}\tensor \ket{f_l}\mid k\in[K], l\in[L]\}$ is an ONB of  $\mcH_1\tensor \mcH_2$. Then for any $k\in[K]$ and $l\in[L]$ we have $$\bra{e_k\tensor f_l}A\tensor B\ket{e_k\tensor f_l}=\bra{e_k}A\ket{e_k}\bra{f_l}B\ket{f_l}\geq 0$$Now let 
	$$A\ket{e_i}=\sum_{j=1}^K\alpha_{i,j}\ket{e_j}\qquad\text{and}\qquad B\ket{f_i}=\sum_{j=1}^L\beta_{i,j}\ket{f_j}$$
	
	Given that for all $\ket{\phi}\in\mcH_1$, $\bra{\phi}A\ket{\phi}\geq 0$ and for all $\ket{\psi}\in\mcH_2$, $\bra{\psi}B\ket{\psi}\geq0$. Now let $\ket{u}\in\mcH_1\tensor \mcH_2$. Then we can write $$\ket{u}=\sum_{k=1}^K\sum_{l=1}^L\gm_{k,l}\ket{e_k}\tensor \ket{f_l}$$
	Then \begin{align*}
		\bra{u}A\tensor B\ket{u} & = \lt[ \sum_{k=1}^K\sum_{l=1}^L\gm_{k,l}^*\bra{e_k}\tensor \bra{f_l} \rt]A\tensor B\lt[ \sum_{k'=1}^K\sum_{l'=1}^L\gm_{k',l'}\ket{e_{k'}}\tensor \ket{f_{l'}} \rt]\\
		& = \sum_{k=1}^K\sum_{l=1}^L\sum_{k'=1}^K\sum_{l'=1}^L\gm_{k,l}^*\gm_{k',l'}\bra{e_k}A\ket{e_{k'}}\bra{f_l}B\ket{f_{l'}}\\
		& = \sum_{k=1}^K\sum_{l=1}^L\sum_{k'=1}^K\sum_{l'=1}^L\gm_{k,l}^*\gm_{k',l'}\bra{e_k}\lt[\sum_{j=1}^K\alpha_{k',j}\ket{e_j}\rt]\bra{f_l}\lt[ \sum_{j=1}^L\beta_{l',j}\ket{f_j} \rt]\\
		& = \sum_{k=1}^K\sum_{l=1}^L\sum_{k'=1}^K\sum_{l'=1}^L\gm_{k,l}^*\gm_{k',l'}\alpha_{k',k}\bra{e_k}\ket{e_k}\beta_{l',l}\bra{f_l} \ket{f_l} 
	\end{align*}
}
