...
$$\bbP\left(\bigcup_{i=1}^{k+1}A_i\right)=\bbP\left(\bigcup_{i=1}^k A_i\right)+\bbP\left(A_{k+1}\setminus\bigcup_{i=1}^k A_i\right)=\bbP\left(\bigcup_{i=1}^k A_i\right)+\bbP(A_{k+1})-\bbP\left(\bigcup_{i=1}^k (A_i\cap A_{k+1})\right)$$Now using inductive hypothesis we have \begin{align*}
	\bbP\left(\bigcup_{i=1}^k (A_i\cap A_{k+1})\right) & =\sum_{t=1}^k(-1)^{t-1}\sum_{J\subseteq [k],\ |J|=t}\bbP\lt[ \bigcap_{i\in J}(A_i\cap A_{k+1}) \rt]\\
	& = \sum_{t=1}^k(-1)^{t-1}\sum_{J\subseteq [k],\ |J|=t}\bbP\lt[ A_{k+1}\cap\lt( \bigcap_{i\in J}A_i \rt)\rt]
...
=\ &  \bbP\left(\bigcup_{i=1}^k A_i\right)+\bbP(A_{k+1})-\lt[ \sum_{t=1}^k(-1)^{t-1}\sum_{J\subseteq [k],\ |J|=t}\bbP\lt[ A_{k+1}\cap\lt( \bigcap_{i\in J}A_i \rt)\rt]  \rt]\\
=\ & \sum_{t=1}^{k}(-1)^{t-1}\sum_{T\subseteq [k],\ |T|=t}\bbP\lt[\bigcap_{i\in T}A_i\rt]+\bbP[A_{k+1}]+\sum_{t=1}^k(-1)^{t}\sum_{J\subseteq [k],\ |J|=t}\bbP\lt[ A_{k+1}\cap\lt( \bigcap_{i\in J}A_i \rt)\rt]\\
=\ & \sum_{i=1}^{k+1}\bbP[A_i]+\sum_{t=1}^k(-1)^{t}\sum_{T\subseteq [k],\ |T|=t+1}\bbP\lt[\bigcap_{i\in T}A_i\rt]+\sum_{t=1}^k(-1)^{t}\sum_{J\subseteq [k],\ |J|=t}\bbP\lt[ A_{k+1}\cap\lt( \bigcap_{i\in J}A_i \rt)\rt]\\
=\ & \sum_{i=1}^{k+1}\bbP[A_i]+\sum_{t=1}^k(-1)^{t}\lt( \sum_{T\subseteq [k],\ |T|=t+1}\bbP\lt[\bigcap_{i\in T}A_i\rt]+\sum_{J\subseteq [k],\ |J|=t}\bbP\lt[ A_{k+1}\cap\lt( \bigcap_{i\in J}A_i \rt)\rt]\rt)\\
=\ &  \sum_{i=1}^{k+1}\bbP[A_i]+\sum_{t=1}^k(-1)^{t}\sum_{T\subseteq [k+1],\ |T|=t+1}\bbP\lt[\bigcap_{i\in T}A_i\rt]\\
=\ & \sum_{t=1}^{k+1}(-1)^{t-1}\sum_{T\subseteq [k+1],\ |T|=t}\bbP\lt[\bigcap_{i\in T}A_i\rt]
\end{align*}
\item Suppose $\om\in\Om$. We will count the contribution of $\om$ to the probability $\bbP\lt[\bigcup\limits_{i=1}^nA_i\rt]$. Now if $\om \notin\bigcup_{i=1}^nA_i$ then $\om$ has no contribution to the probability $\bbP\lt[\bigcup\limits_{i=1}^nA_i\rt]$. Now we also have $\om\notin \bigcap\limits_{i\in J}A_i$ for all $J\subseteq [n]$. Hence $\om$ has no contribution to any of the $S_i$ for all $i\in[n]$.\parinn
