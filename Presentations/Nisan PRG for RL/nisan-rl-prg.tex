...
\end{frame}
\begin{frame}[allowframebreaks]{Algorithm always returns an $h$}
	\begin{claim}
	...
	\end{claim}
	Let $A_{i,j}$ be the set of $r\in\{0,1\}^k$ such that $Q(i;\mcG_{s-1}(r,h_1,\dots, h_{s-1}))=j$. So $M_{h_1,\dots, h_{s-1}}[i,j]=\rho(A_{i,j})$ where $\rho(A)=|A|/2^k$.


	\begin{itemize}
		\item For any $i,j\in[N]$, $$M^2_{h_1,\dots, h_{s-1}}=\sum_{l\in[N]}\rho(A_{i,l})\cdot \rho(A_{l,j})$$
	...
	For a random $h\in\mcH$ with probability at least $1-\frac{N^4}{2^k\delta^2}\geq 1-\frac1{2n^3}$, $$\left|\underset{r\in\{0,1\}^k}{\bbP}[r\in A_{i,l}\wedge h(r)\in A_{l,j}]-\rho(A_{i,l})\cdot \rho(A_{l,j})\right|\leq \frac{\rho(A_{i,l})\cdot \rho(A_{l,j})}{k^2}\leq \frac{\delta}{N^2}$$

	So by Union Bound random $h\in \mcH$, $(\frac{\dl}{N^2},A,B)$-good for all $A,B$ with probability at least $\frac12$.
...
\begin{frame}{Setting Parameters}
	\begin{itemize}
		\item Set $k=\log(n^c)=O(\log n)$. So $t\approx n^c$.
	 \vfill 
