...
\begin{frame}{List Decoding}
	...
	\begin{itemize}
		\item Combinatorial List Decoding: Prove if $\textcolor{blue}{|L(v)|=\poly(n)}$\pause
	...
	We will talk in terms of agreement instead of errors
...
\begin{frame}{Reed Solomon Codes}
	...
	$$\textcolor{blue}{(f(\alpha_1),f(\alpha_2), \dots, f(\alpha_n))}$$\pause
...
\begin{frame}{Reed Solomon Decoding History}
	...
	Deterministic unique decoding upto $\textcolor{blue}{(n-k+1)/2}$ errors is possible using \textcolor{blue}{Berlekamp-Welch} algorithm in $\textcolor{blue}{\poly(n,\log q)}$ time.\pause
...
\begin{frame}{Factorization Barrier}
	Sudan and Guruswami–Sudan algorithms rely on the factorization of bivariate polynomials over finite fields\pause
	\begin{itemize}
