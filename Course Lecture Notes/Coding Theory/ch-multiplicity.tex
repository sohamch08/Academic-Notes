...
\begin{definition}[Multiplicity]
	For $P(\ovX)\in \bbF[\ovX]$ and $\ova\in \bbF^m$ the multiplicity of $P$ at $\ova\in \bbF^m$ denoted $mult(P,\ova)$ is the largest integer $M$ such that for every non-negative vector $\ovi$ with $wt(\ovi)<M$ we have $P^{(\ovi)}(\ova)=0$ (If $M$ may be taken arbitrarily large we set $mult(P,\ova)=\infty$)
\end{definition}
Note that $mult(P,\ova)\geq 0$ for all $\ova\in \bbF^m$.
\subsection{Basic Properties of Multiplicity}
We now translate some properties of the Hasse derivative into properties of the multiplicities. We will discuss the properties of multiplicities from \cite{dvir2009extensions}
