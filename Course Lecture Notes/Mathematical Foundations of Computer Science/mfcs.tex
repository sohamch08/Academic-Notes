...
\definecolor{mycolor1}{HTML}{1EAE81}
...
\newcommand*{\titre}[2]{\begingroup
	...
	\centering
	\settowidth{\unitlength}{\Huge\scshape\bfseries CSS.102.1 Mathematical Foundations\hspace{3pt} \hspace{10pt}}
	\vspace*{\baselineskip}
...
\begin{titlepage}
	...
	\titre{CSS.102.1 Mathematical Foundations\\[3mm] of Computer Science: Combinatorics}{Raghuvansh Saxena}
\end{titlepage}
...
\begin{Definition}{Projective Planes}{}
	Let $n>0$ and $S_1,\dots, S_m\subseteq [n]$ are a projective plane if \begin{enumerate}
		\item $|S_i|\geq 2$ for all $i\in[m]$ 
		\item For all $i\neq j\in[n]$ $\exs!\ k\in[m]$ such that $i,j\in S_k$ 
		\item For all $i\neq j\in [m]$, $S_i\cap S_j=\{j\}$ for some $j\in[n]$. 
		\item $\exs$ distinct $j_1,j_2,j_3,j_4\in[n]$ such that for all $i\in[n]$, $|S_i\cap\{j_1,j_2,j_3,j_4\}|\leq 2$. 
	\end{enumerate}
\end{Definition}
The elements in $[n]$ are called points and the subsets $S_i\subseteq [n]$ for all $i\in[m]$ are called lines. 
\begin{Example}{Fano System}{}
	For $n=7$ consider the following sets 
	
	\begin{minipage}{0.69\textwidth}
		\begin{align*}
			S_1=\{1,3,2\}, && &S_2=\{2,6,4\}, && S_3=\{4,5,1\},\\ 
			S_4=\{1,7,6\}, && &S_5=\{2,7,5\}, && S_6=\{4,7,3\},\\ 
			&& &S_7=\{3,5,6\} &&  
		\end{align*}
	\end{minipage}
	\begin{minipage}{0.3\textwidth}
		\begin{center}
			...
			\begin{tikzpicture}[scale=2.3]
				...
				\node[draw,circle] (G) at (0,0) {7};
				\node[draw,circle] (A) at (0,1) {1};
				\node[draw,circle] (B) at (-0.866,-0.5) {2};
				\node[draw,circle] (C) at (0.866,-0.5) {4};
				\node[draw,circle] (D) at (0,-0.5) {6};
				\node[draw,circle] (E) at (0.433,0.25) {5};
				\node[draw,circle] (F) at (-0.433,0.25) {3};
				% Draw a dashed circle with gaps around D, E, F
				\draw (G) circle [radius=0.5];
				\node[draw,circle,fill=myexamplebg] (D) at (0,-0.5) {6};
				\node[draw,circle,fill=myexamplebg] (E) at (0.433,0.25) {5};
				\node[draw,circle,fill=myexamplebg] (F) at (-0.433,0.25) {3};
				\draw (A) -- (F)--(B) -- (D) -- (C) -- (E) -- (A) -- cycle;
