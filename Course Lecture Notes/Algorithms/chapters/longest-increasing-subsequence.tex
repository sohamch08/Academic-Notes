...
\section{\texorpdfstring{$O(n\log n)$}{O(logn)} Time Algorithm}
...
\begin{lemma}{}{}
At any time $t$, $M[1]\leq M[2]\leq \cdots\leq M[n]$
\end{lemma}
\begin{proof}
We will prove this by induction on $t$. The base case follows naturally. Now for $k^{th}$ iteration suppose $M[k]$ is replaced by $x_i$. Then we know $\forall \ j<k$ we have $M[j]\leq x_i$. By inductive hypothesis at time $t-1$ we have $M$ as an increasing sequence. Now before replacing $M[k]\leq M[k+1]\leq \cdots M[n]$. Now by \lmref{th:mdecreasing} $M[k]$ is nonincreasing. So  So we still have $M[1]\leq \cdots M[k-1]\leq x_i\leq M[k+1]\leq \cdots \leq M[n]$. Hence bt mathematical induction it holds.
\end{proof}
\begin{lemma}{}{}
	After any $i^{th}$ iteration, for $k\in[n]$ if $M[k]<\infty$ then $M[k]$ stores the smallest value in $x_1,\dots, x_n$ such that there is an increasing subsequence of size $k$ that ends in $M[k]$.
\end{lemma}