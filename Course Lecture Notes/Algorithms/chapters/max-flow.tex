...
N3121ow define for all $v\in V$ and for all preflow $f$, $\textit{excess}_f(v)=\sum\limits_{e\in\textit{in}(v)}f(e)-\sum\limits_{e\in\textit{out}(v)}f(e)$. If $f$ is a preflow then $\textit{excess}_f(s)\leq 0$ and $\forall\ v\in V\setminus\{s\},$ $\textit{excess}_f(v)\geq 0$
\begin{lemma}{}{}
...
\begin{itemize}[label=$\bullet$]
	\item 	Step 1:
		...
		\draw (4,-0.5) node{$0$};
		\draw (0,-1) node{$-6$} node[left=0.5cm]{$\textit{excess}:$};
		\draw (2,-1) node{$6$};
		\draw (4,-1) node{$0$};
		...
		\draw (10,0) node[text width=10cm]{Since $\textit{excess}_ff(v)=6>0$. So in first iteration $v$ is taken. Since there is no edge $(v,u)$ with $l(u)=l(v)-1$, label of $v$ got increased};
	\end{tikzpicture}
\item 	Step 2:
...
\end{observation}
Now by \lmref{satpush2mn} total number of saturated pushes is at most $2mn$. And by \corref{eachvertexlabel2n} each vertex has label at most $2n$. Hence in total $\phi(f)$ can increase at most $2mn\times 2n=4mn^2$ by saturated pushes.
