...
\begin{itemize}[label=$\bullet$]
	\item 	Step 1:
	      ...
	      \begin{tikzpicture}
		      ...
		      \draw (10,0) node[text width=10cm]{Since $\emph{excess}_f(v)=6>0$. So in first iteration $v$ is taken. Since there is no edge $(v,u)$ with $l(u)=l(v)-1$, label of $v$ got increased};
	      \end{tikzpicture}
	\item 	Step 2:
	      ...
	      \begin{tikzpicture}
		      ...
		      \draw (10,0) node[text width=10cm]{Since $\emph{excess}_ff(v)=2>0$, in second iteration again  $v$ is selected.  There is an edge $(v,t)$ with $l(t)=0=l(v)-1=1-1$. Now $\dl=c_f(v,t)=4$. Hence, saturating push. The preflow gets updated,  $f(s,v)=6$, $f(v,t)=4$.};
	      \end{tikzpicture}
	\item Step 5:
	      ...
	      \begin{tikzpicture}
		      ...
		      \draw ($(s)+(-0.5,0.7)$) node[left]{$f:$};
		      \draw[-latex] ($(v)+(0.3,0.7)$) -- ($(t)-(0.3,-0.7)$) node[above, midway]{$4$};
		      \draw (10,0) node[text width=10cm]{Since $\emph{excess}_ff(v)=2>0$,  in next 3 iterations again  $v$ is selected. Since there is no edge $(v,u)$ with $l(u)=l(v)-1$, label of $v$ gets increased every time. Which becomes $4$ after $3$ iterations.};
	      \end{tikzpicture}
	      %\item 
	\item Step 6:
	      ...
	      \begin{tikzpicture}
		      ...
		      \draw ($(s)+(-0.5,0.7)$) node[left]{$f:$};
		      \draw[-latex] ($(v)+(0.3,0.7)$) -- ($(t)-(0.3,-0.7)$) node[above, midway]{$4$};
		      \draw (10,0) node[text width=10cm]{Since $\emph{excess}_ff(v)=2>0$,  in this iteration again  $v$ is selected. There is an edge $(v,s)$ with $l(t)=3=l(v)-1=4-1$. Now $\dl=\emph{excess}_f(v,s)=2$. Hence, it's non-saturating push. So the preflow gets updated $f(s,v)=6-2=4$, $f(v,t)=4$. Now it's a valid flow. Now there is no vertex with postive \emph{excess}. Hence, the algorithm stops.};
	      \end{tikzpicture}
\end{itemize}
%\end{example}
