...
%\handout{1: Bipartite Matching}{T. Kavitha}{Soham Chatterjee}
...
\end{algorithm}
\subsubsection{Hungarian Forest}
\begin{figure}[h]
	\centering
	\begin{tikzpicture}                                                 
...
Now we have to  show the algorithm always outputs a augmenting path. Instead of showing that we will show if the algorithm can not find an $M$-augmenting path then $M$ is a maximum matching. We will show that using vertex cover.
