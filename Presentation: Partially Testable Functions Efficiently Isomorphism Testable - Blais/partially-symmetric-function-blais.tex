...
% Dados
%---------------------------------------------------------------------
\title{Partially Symmetric Functions are Efficiently Isomorphism Testable}
%opçao para 1 autor
...
\vspace{3mm}
 ...
 Base Case ($g=G+1$): We have $x< 2^{2(G+1)-1}=2^{2G+1}=2^{n+1}$. True as $x$ is $n$-bit number. And second condition follows by constuction.
...
\hspace{0.5cm} $x_k=(x_{k+1}\bmod{m_{2k}})\bmod{m_{2k-1}}$. $m_{2k-1}=2^{2k-1}$. So $x_k$ has $2k-1$ bits so $ x_k<2^{2k-1}$. So Part (a) is proved.
\framebreak
...
\begin{itemize}
	...
	\item $1\leq g\leq G$  $$m_{2g}=4^{2g}+4^{2g-1}+\sum_{\substack{j\text{th edge}\\ \text{out-edge from }g}}4^j$$ $$m_{2g-0.5}=4^{2g}-4^{2g-1}=3\times 4^{2g-1},\ \ m_{2g-1}=4^{2g-1}$$
\end{itemize}
\framebreak
...
\textbf{If at least one of them 0}:
 ...
 $x_{k+1}\bmod{m_{2k}}=x_{k+1}$. In $x_k$ positions where $m_{2k}$ has 1 will have 1. $$x_{k+1}=a\times 4^{2k}+b\times 4^{2k-1}+c\text{ where }a,b\in \{0,1\}$$\begin{itemize}
	...
	\item $a=0$: $$(x_{k+1}\bmod{m_{2k-0.5}})\bmod{m_{2k-1}}=b\times 4^{2k-1}+c\bmod{m_{2k-1}}=c$$
\end{itemize}
\framebreak
